...
type FormType = PassengerTypeClass
const { item } = defineProps<{ item?: FormType }>()

const { PassengerTypeClassSchema: schema } = useZodSchema()
...
const {
  ...
  submit: clientStore.submitPassengerTypeClass,
  state: {
    active: true,
    fee: {
      type: 'percent-min',
  ...
  meta: {
    category: 'Passenger Type Class'
  }
})

const formEl = ref()
const test = () => {
  console.log('errors: ', formEl.value.getErrors())
